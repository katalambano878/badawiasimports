...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.body || 'New update from Badawias Imports',
    icon: '/logo.svg',
    badge: '/logo.svg',
    vibrate: [100, 50, 100],
    data: {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Badawias Imports', options)
  );
});
